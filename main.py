...
# In payout mode, cap payout (NOT stake)
MARTINGALE_MAX_PAYOUT = 8.00
    ...
    async def scan_market(self, symbol: str):
        while self.is_scanning:
            try:
                ...
                if len(candles) < 70:
                    self.market_debug[symbol] = {"time": time.time(), "gate": "Waiting candles", "why": [f"need ~70, have {len(candles)}"]}
                ...
                avg_body = float(np.mean(bodies)) if len(bodies) >= 10 else 0.0
                ...
                call_ready = (
                    uptrend and slope_ok and ema50_rising and touched_ema20 and bull_confirm and close_above_ema20
                    and call_rsi_ok and (not spike_block) and (not flat_block)
                )

                put_ready = (
                    downtrend and slope_ok and ema50_falling and touched_ema20 and bear_confirm and close_below_ema20
                    and put_rsi_ok and (not spike_block) and (not flat_block)
                )
    ...
    async def execute_trade(self, side: str, symbol: str, reason="MANUAL", source="MANUAL"):
        ...
        async with self.trade_lock:
            ...
            try:
                ...
                # ✅ PAYOUT MODE PROPOSAL (Deriv calculates stake)
                ...
                buy = await self.api.buy({"buy": prop["proposal"]["id"], "price": stake_used})
        ...
        try:
            ...
            if source == "AUTO":
                ...
                if profit <= 0:
                    ...
                    self.current_payout = min(float(payout_used) * MARTINGALE_MULT, MARTINGALE_MAX_PAYOUT)
                else:
                    self.consecutive_losses = 0
                    self.martingale_step = 0
                    self.current_payout = self.base_payout
...
def main_keyboard():
    mode = "✅ PAYOUT" if bot_logic.use_payout_mode else "❌ PAYOUT"
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(mode, callback_data="TOGGLE_PAYOUT")],
        [InlineKeyboardButton("➖ Payout -0.10", callback_data="PAYOUT_DOWN"),
         InlineKeyboardButton("➕ Payout +0.10", callback_data="PAYOUT_UP")]
    ])
...
async def btn_handler(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "SET_DEMO":
    ...
    elif q.data == "PAYOUT_UP":
        bot_logic.base_payout = round(bot_logic.base_payout + 0.10, 2)
        bot_logic.base_payout = max(bot_logic.base_payout, MIN_PAYOUT)
        bot_logic.current_payout = bot_logic.base_payout
        bot_logic.martingale_step = 0
        await q.edit_message_text(f"✅ Base payout set to ${bot_logic.base_payout:.2f}", reply_markup=main_keyboard())

    elif q.data == "PAYOUT_DOWN":
        bot_logic.base_payout = round(bot_logic.base_payout - 0.10, 2)
        bot_logic.base_payout = max(bot_logic.base_payout, MIN_PAYOUT)
        bot_logic.current_payout = bot_logic.base_payout
        bot_logic.martingale_step = 0
        ...
        header = (
            ...
            f"💵 Today PnL: {bot_logic.total_profit_today:+.2f} / +{DAILY_PROFIT_TARGET:.2f}\n"
            f"🚦 Gate: {gate}\n"
            f"💰 Balance: {bot_logic.balance}\n"
        )
        await q.edit_message_text(header, reply_markup=main_keyboard())
