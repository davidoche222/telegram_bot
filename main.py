...
APP_ID = 1089
MARKETS = ["R_50", "R_75", "R_100"]   # R_10 removed — poor performance
COOLDOWN_SEC = 3
...
# ===== STRATEGY =====
TF_SEC = 60
CANDLES_COUNT = 150          # FIX 1: was 100 — ATR50 needs more candles to warm up
DURATION_MIN = 2
MA_PERIOD = 50
STD_PERIOD = 20
ATR_SHORT = 14
...
# ===== Dynamic Z threshold per market =====
Z_THRESHOLDS = {
...
PERF_TUNE_LOOKBACK = 15
PERF_TUNE_MIN_WINRATE = 0.50
PERF_TUNE_Z_BOOST = 0.3
...
# ===== INDICATORS =====
def calculate_zscore(closes, ma_period=50, std_period=20):
...
def get_adaptive_expiry(z_abs):
    if z_abs>=EXPIRY_STRONG_Z_THRESHOLD: return EXPIRY_STRONG_MIN
...
class DerivSniperBot:
    ...
    def get_tuned_z_threshold(self,symbol):
        ...
        if len(recent)>=PERF_TUNE_LOOKBACK:
            ...
            if wr<PERF_TUNE_MIN_WINRATE:
                ...
                logger.info(f"⚠️ {symbol} win rate {wr:.0%} — Z tightened to {thresh['min']:.1f}")
        return thresh
    ...
    def get_best_markets(self):
        ...
        scored=[]
        for m in MARKETS:
            if m in by_mkt and by_mkt[m]["trades"]>=5: scored.append((m,wr(by_mkt[m])))
            else: scored.append((m,50.0))
    ...
    def _equity_ok(self):
        ...
        if ratio<=EQUITY_HALF_STAKE_PCT:
            return True,f"Equity warning: balance at {ratio:.0%} — stake halved",0.5
        return True,"OK",1.0
    ...
    def can_trade_market(self,symbol):
        ...
        if self.market_recently_losing(symbol):
            return False,f"Market warm-up: {symbol} had 3+ losses in last 5 trades"
        return True,"OK"
        ...
        while self.is_scanning:
            try:
                ...
                if self.candle_cooldown_counter.get(symbol,0)>0:
                    ...
                    self.market_debug[symbol]={"time":time.time(),"gate":gate,"z_now":round(z_now,4),"z_prev":round(z_prev,4),"vol_expanding":vol_exp,"spike":spike,"why":[f"Candle cooldown: {self.candle_cooldown_counter[symbol]+1} left"]}
                    self.last_processed_closed_t0[symbol]=confirm_t0; continue
                ...
                # FIX 3: Duplicate PUT block removed — clean execution
                if signal=="CALL":
        ...
        async with self.trade_lock:
            ...
            try:
                ...
                await self.safe_send_tg(
                    ...
                    f"⏱ Expiry: {expiry}m {'(strong signal)' if expiry==EXPIRY_STRONG_MIN else ''}\n"
                    f"🎁 Payout: ${payout:.2f}\n"
                    f"📐 Z-Score: {z_now:.4f} (±{z_thresh['min']:.1f} / cap ±{z_thresh['max']:.1f})\n"
    ...
    async def check_result(self,cid,source,side,z_now,expiry=2):
            ...
            if source=="AUTO":
                ...
                if profit<=0:
                    ...
                    if self.consecutive_losses>=5 and not self.hit_5_losses_today:
                        ...
                        await self.safe_send_tg("⛔ STOPPED: 5 consecutive losses. Bot paused until tomorrow.")
                ...
                if self.total_profit_today>=PROFIT_LOCK_TRIGGER and not self.profit_lock_active:
                    ...
                    await self.safe_send_tg(f"🔒 PROFIT LOCK ACTIVATED at +${self.total_profit_today:.2f} — floor +${PROFIT_LOCK_FLOOR:.2f}")
                if self.total_profit_today>=DAILY_PROFIT_TARGET:
                    self.pause_until=self._next_midnight_epoch()
            await self.fetch_balance()
...
async def cmd_setstake(u:Update,c:ContextTypes.DEFAULT_TYPE):
    ...
    except: await u.message.reply_text("Usage: /setstake 0.50")
...
async def cmd_setmarkets(u:Update,c:ContextTypes.DEFAULT_TYPE):
    ...
    except: await u.message.reply_text("Usage: /setmarkets R_50 R_25")
...
async def cmd_resume(u:Update,c:ContextTypes.DEFAULT_TYPE):
    bot_logic.pause_until=0.0; bot_logic.section_pause_until=0.0
...
async def btn_handler(u:Update,c:ContextTypes.DEFAULT_TYPE):
    ...
    if q.data=="SET_DEMO":
        ...
        await _safe_edit(q,"🔍 SCANNER ACTIVE\n✅ Z-Score v5 running.\n📡 Markets: R50, R75, R100",reply_markup=main_keyboard())
...
async def start_cmd(u:Update,c:ContextTypes.DEFAULT_TYPE):
    await u.message.reply_text(
        "💎 Deriv Z-Score Bot v5\n"
        f"🧭 Strategy: Z-Score Mean Reversion\n"
        f"📐 MA{MA_PERIOD}/STD{STD_PERIOD} | Dynamic Z per market\n"
        f"🛡 ATR{ATR_SHORT}/{ATR_LONG} filter | Spike blocker | Z cap\n"
        f"💰 Equity protection | Profit lock | Session loss limits\n"
        f"📊 Auto market rotation | Performance Z tuning\n"
