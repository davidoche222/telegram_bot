...
def calculate_indicators(candles):
    ...
    if len(ema21) < 2 or len(ema50) < 3: 
        return None

    rsi_vals = calculate_rsi(c, 14)
    ema21_slope = ema21[-1] - ema21[-2]
    ema50_slope = ema50[-1] - ema50[-3]
...
class DerivSniperBot:
    ...
    async def scan_market(self, symbol: str):
        while self.is_scanning:
            ...
            try:
                ...
                if ind["c"] > ind["ema50"] and ind["ema50_slope"] > 0: 
                    if ind['ema9'] > ind['ema21'] and ind['ema21_slope'] > 0: 
                        if ind['c'] > ind['ema21']: 
                            if 45 <= ind['rsi'] <= 60 and ind['rsi'] > ind['rsi_prev']: 
                                if ind['c'] > ind['ema9']: 
                                    if 35 <= ind['rsi'] <= 65: 
                                        await self.execute_trade("CALL", symbol, "EMA50 Trend + EMA Cross + RSI Rising", source="AUTO")

                elif ind["c"] < ind["ema50"] and ind["ema50_slope"] < 0: 
                    if ind['ema9'] < ind['ema21'] and ind['ema21_slope'] < 0: 
                        if ind['c'] < ind['ema21']: 
                            if 40 <= ind['rsi'] <= 55 and ind['rsi'] < ind['rsi_prev']: 
                                if ind['c'] < ind['ema9']: 
                                    if 35 <= ind['rsi'] <= 65: 
                                        await self.execute_trade("PUT", symbol, "EMA50 Trend + EMA Cross + RSI Falling", source="AUTO")
    ...
    async def execute_trade(self, side: str, symbol: str, reason="MANUAL", source="MANUAL"):
        ...
        async with self.trade_lock:
            try:
                # Use current_stake for Martingale
                stake_to_use = self.current_stake if source == "AUTO" else self.base_stake
        ...
        try:
            ...
            if source == "AUTO":
                if profit <= 0:
                    ...
                    self.consecutive_losses = 0
                    self.current_stake = self.base_stake # Reset Martingale
            await self.fetch_balance()
