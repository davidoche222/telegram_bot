...
TARGET_PAYOUT = 1.00  # Sets risk to ~$0.52 to get ~$0.48 interest
COOLDOWN_SEC = 300 
        ...
        try:
            ...
            if adx_v < 25 and time.time() >= self.cooldown_until:
                if lo <= lw and pk < 20 and pk > pd and cl > op:
                    await self.execute_trade("CALL", symbol, f"Ranging Market (ADX:{adx_v:.1f})\nBottom Band Touch + Stoch Cross ({pk:.1f})")
                elif hi >= up and pk > 80 and pk < pd and cl < op:
                    await self.execute_trade("PUT", symbol, f"Ranging Market (ADX:{adx_v:.1f})\nTop Band Touch + Stoch Cross ({pk:.1f})")
    ...
    async def execute_trade(self, side: str, symbol: str, reason="MANUAL"):
        ...
        async with self.trade_lock:
            try:
                ...
                interest = TARGET_PAYOUT - stake_used
...
async def start_cmd(u: Update, c: ContextTypes.DEFAULT_TYPE):
    await u.message.reply_text("💎 **Sniper Range M1 v4.8**", reply_markup=main_keyboard())
