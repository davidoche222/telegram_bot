...
class DerivBot:
    ...
    def reset_day_if_needed(self):
        ...
        if self.day_key != key:
            self.day_key, self.trades_today_total = key, 0
            self.losses_today_total, self.wins_today_total = 0, 0
            self.trades_today_by_symbol = {m: 0 for m in MARKETS}
            self.losses_today_by_symbol = {m: 0 for m in MARKETS}
            self.total_profit_today, self.current_stake, self.consecutive_losses = 0.0, BASE_STAKE, 0
    ...
    async def fetch_balance(self):
        ...
        try:
            ...
            self.balance = f"{bal_value:.2f} {bal['balance']['currency']}"
            self.reset_day_if_needed()
            if self.start_balance_value is None: self.start_balance_value = bal_value
        except: pass
    ...
    def gate(self, symbol: str):
        ...
        if now < self.symbol_chop_until.get(symbol, 0.0): return False, "Market is Choppy (Doji filter)"
        if self.active_trade_info: return False, "Active trade in progress"
        return True, "OK"
        ...
        while self.is_scanning:
            try:
                ...
                if len(struct) < 50 or len(m1) < 60:
                    self.market_debug[symbol] = {"time": time.time(), "bias": "…", "setup": "-", "signal": "-", "levels": "", "ind": "", "waiting": "Syncing Data..."}
                ...
                ind_txt = f"RSI:{rsi_now:.0f} | Doji:{dojis}/10"
                
                wait_reason = "Waiting for setup"
                if bias == "RANGE": wait_reason = "Structural Trend is Neutral"
                elif not trend_sep_ok: wait_reason = "EMA Separation too low"
                elif not touched_ema20: wait_reason = "Price hasn't touched EMA20 zone"
                elif bias == "UP" and not rsi_buy_ok: wait_reason = f"RSI {rsi_now:.0f} too weak for BUY"
                elif bias == "DOWN" and not rsi_sell_ok: wait_reason = f"RSI {rsi_now:.0f} too weak for SELL"
                elif sig_rng > SPIKE_MULTIPLIER * avg_rng: wait_reason = "Spike candle detected"
    ...
    async def execute_trade(self, side: str, symbol: str, reason: str):
        async with self.trade_lock:
            ...
            if not ok: return
            try:
                prop = await self.api.proposal({"proposal": 1, "amount": self.current_stake, "basis": "stake", "contract_type": side, "currency": "USD", "duration": int(EXPIRY_MIN[symbol]), "duration_unit": "m", "symbol": symbol})
                ...
                await self.app.bot.send_message(TELEGRAM_CHAT_ID, f"🚀 {side} OPENED (${self.current_stake})\n🛒 {symbol}\n🧠 {reason}")
    ...
    async def check_result(self, cid: int, symbol: str):
        ...
        try:
            ...
            await self.fetch_balance()
            await self.app.bot.send_message(TELEGRAM_CHAT_ID, f"🏁 {'WIN' if profit > 0 else 'LOSS'} ({profit:+.2f})\n💰 Bal: {self.balance}")
...
async def btn_handler(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "SET_DEMO":
        ...
        await bot_logic.connect(); await q.edit_message_text("✅ DEMO Connected", reply_markup=main_keyboard())
    elif q.data == "SET_REAL":
        bot_logic.active_token, bot_logic.account_type = REAL_TOKEN, "LIVE"
        await bot_logic.connect(); await q.edit_message_text("⚠️ LIVE Connected", reply_markup=main_keyboard())
    elif q.data == "START_SCAN":
        bot_logic.is_scanning = True
        bot_logic.scanner_task = asyncio.create_task(bot_logic.background_scanner())
        await q.edit_message_text("🔍 SCANNING ACTIVE", reply_markup=main_keyboard())
    elif q.data == "STOP_SCAN":
        bot_logic.is_scanning = False; await q.edit_message_text("⏹️ STOPPED", reply_markup=main_keyboard())
    elif q.data == "STATUS":
        await bot_logic.fetch_balance()
        status_msg = (f"🤖 Bot: {'ON' if bot_logic.is_scanning else 'OFF'} | Today: ✅{bot_logic.wins_today_total} ❌{bot_logic.losses_today_total}\n"
                      f"💰 Bal: {bot_logic.balance}\n━━━━━━━━━━━━━━━\n")
        for sym in MARKETS:
            status_msg += fmt_scan(sym, bot_logic.market_debug.get(sym, {})) + "\n\n"
        await q.edit_message_text(status_msg, reply_markup=main_keyboard())

async def start_cmd(u: Update, c: ContextTypes.DEFAULT_TYPE):
    await u.message.reply_text("💎 Deriv ATR Bot\nMulti-Timeframe Strategy", reply_markup=main_keyboard())
