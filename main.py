...
# Setting Payout to $1.00 ensures your NET PROFIT is ~$0.48
TARGET_PAYOUT = 1.00  
COOLDOWN_SEC = 300 
...
class DerivSniperBot:
        ...
        try:
            ...
            if adx_v < 25 and time.time() >= self.cooldown_until:
                if lo <= lw and pk < 20 and pk > pd and cl > op:
                    await self.execute_trade("CALL", symbol, f"Bottom BB + Stoch ({pk:.1f})")
                elif hi >= up and pk > 80 and pk < pd and cl < op:
                    await self.execute_trade("PUT", symbol, f"Top BB + Stoch ({pk:.1f})")
    ...
    async def execute_trade(self, side: str, symbol: str, reason="MANUAL"):
        ...
        async with self.trade_lock:
            try:
                # Switching to PAYOUT basis to target exactly $0.48 profit
                ...
                net_profit = TARGET_PAYOUT - stake_used

                buy = await self.api.buy({"buy": proposal["proposal"]["id"], "price": stake_used + 0.05})
                ...
                msg = (f"🚀 **{side} TRADE OPENED**\n"
                       f"🛒 Market: `{symbol}`\n"
                       f"📉 Risk (Stake): `${stake_used:.2f}`\n"
                       f"💰 Net Profit Target: `${net_profit:.2f}`\n"
                       f"📝 **Reason**: {reason}")
...
async def start_cmd(u: Update, c: ContextTypes.DEFAULT_TYPE):
    await u.message.reply_text("💎 **Sniper Range M1 v4.6**", reply_markup=main_keyboard())
